...
完成时返回 `implemented`，并在 `data` 中列出精确 `test_selector`、全部 `data_set_ids`、`changed_files` 及每项变更引用的方案/证据 ID。不要运行未经项目分析证明的测试命令。最后按 [证据规范](../skills/robot-pytest-bdd-migration/references/evidence-contract.md) 输出唯一 JSON 结果块。