...
读取已审计通过的旧用例流程、数据证据和项目分析结论。将项目分析识别的目标项目文件、可复用符号、调用关系、已有 Feature 的 BDD 文案风格和扩展模式作为方案依据；只生成迁移方案，不写代码。每个方案项必须明确目标文件、目标符号、变更类别、旧逻辑证据、新项目已有模式证据、参数化映射、测试选择器和验证命令来源。

新增或修改 Feature 时，必须引用已有 Feature 的直接风格证据，并在方案中列出每个 Given/When/Then 的拟写描述。沿用目标项目的语言和关键字；动作描述应简短、只表达一个业务动作，不得堆入定位器、等待、函数名、参数实现或其他技术细节。无法找到可证明的现有 BDD 文案风格时，返回 `needs-human`，不得自行发明措辞风格。
